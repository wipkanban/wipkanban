/**
 * @author https://tools.ietf.org/html/rfc7231#section-6.1
 *
 * Content based on the above RFC site
 */

 /*
 * It indicates that the REST API successfully carried out whatever action the client
 * requested, and that no more specific code in the 2xx series is appropriate. Unlike
 * the 204 status code, a 200 response should include a response body.The information
 ...
 * a collection. There may also be times when a new resource is created as a result
 * of some controller action, in which case 201 would also be an appropriate response.
 ...
 * It indicates that the request has been accepted for processing, but the processing has
 * not been completed. The request might or might not be eventually acted upon, or even
 * maybe disallowed when processing occurs. Its purpose is to allow a server to accept a
 * request for some other process (perhaps a batch-oriented process that is only run once
 * per day) without requiring that the user agent’s connection to the server persist until
 ...
 * the REST API declines to send back any status message or representation in the response
 * message’s body.
 *
 * An API may also send 204 in conjunction with a GET request to indicate that the requested
 * resource exists, but has no state representation to include in the body.
 *
 ...
 * header field. The user agent (e.g. a web browser) is invited by a response with this code
 * to make a second, otherwise identical, request to the new URL specified in the location field.
 *
 * Many web browsers implemented this code in a manner that violated this standard, changing
 * the request type of the new request to GET, regardless of the type employed in the
 * original request (e.g. POST). RFC 1945 and RFC 2068 specify that the client is not allowed
 * to change the method on the redirected request. The status codes 303 and 307 have been
 * added for servers that wish to make unambiguously clear which kind of reaction is expected
 * of the client.
 */
export const FOUND = 302;

/**
 * A 303 response indicates that a controller resource has finished its work, but instead
 *  of sending a potentially unwanted response body, it sends the client the URI of a
 * response resource. This can be the URI of a temporary status message, or the URI to
 * some already existing, more permanent, resource.
 *
 ...
 * The key distinction is that 204 is used when there is nothing to send in the body, whereas
 * 304 is used when the resource has not been modified since the version specified by the
 ...
 * resource. For example, a 307 response can be used to shift a client request over
 * to another host.
 *
 * The temporary URI SHOULD be given by the Location field in the response. Unless the request
 * method was HEAD, the entity of the response SHOULD contain a short hypertext note with
 * a hyperlink to the new URI(s). If the 307 status code is received in response to a request
 ...
 * appropriate. Errors can be like malformed request syntax, invalid request message parameters,
 ...
 * A 403 response is not a case of insufficient client credentials; that would be
 * 401 (“Unauthorized”).
 *
 * Authentication will not help and the request SHOULD NOT be repeated. Unlike a 401 Unauthorized
 ...
 *
 * No indication is given of whether the condition is temporary or permanent. The 410 (Gone) status
 * code SHOULD be used if the server knows, through some internally configurable mechanism,
 * that an old resource is permanently unavailable and has no forwarding address. This status code
 * is commonly used when the server does not wish to reveal exactly why the request has been
 * refused, or when no other response is applicable.
 */
export const NOT_FOUND = 404;

/**
 * The API responds with a 405 error to indicate that the client tried to use an HTTP method that
 * the resource does not allow. For instance, a read-only resource could support only GET and HEAD,
 * while a controller resource might allow GET and POST, but not PUT or DELETE.
 *
 * A 405 response must include the Allow header, which lists the HTTP methods that the resource
 ...
 * request for data formatted as application/xml will receive a 406 response if the API is only
...
/**
 * The 412 error response indicates that the client specified one or more preconditions in its
 * request headers, effectively telling the REST API to carry out its request only if certain
 * conditions were met. A 412 response indicates that those conditions were not met, so instead
 * of carrying out the request, the API sends this status code.
 */
export const PRECONDITION_FAILED = 412;

/**
 * The 415 error response indicates that the API is not able to process the client’s supplied
 * media type, as indicated by the Content-Type request header. For example, a client request
 * including data formatted as application/xml will receive a 415 response if the API is only
 ...
 */
export const UNSUPPPORTED_MEDIA_TYPE = 415;

/**
 * 500 is the generic REST API error response. Most web frameworks automatically respond with
 * this response status code whenever they execute some request handler code that raises an
 * exception.
 *
 ...
 * response.
 *
 * API response is the generic error message, given when an unexpected condition was encountered
