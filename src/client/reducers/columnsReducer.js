...
function getTaskIndex(state, idtask, columnIndex) {
  return state[columnIndex].tasks.findIndex(task => task.id === idtask);
}
...
export default function columnsReducer(state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case actionsType.LOAD_COLUMNS_SUCCESS:
      var columns = action.columns.map(column => {
        return {
          ...column,
    ...
    case actionsType.TOGGLE_INPUT_NEW_TASK:
      indexColumn = state.columns.findIndex(column => column.id === action.id);
    ...
    case actionsType.CREATE_NEW_TASK:
      indexColumn = state.columns.findIndex(
    ...
    case actionsType.UPDATE_TASK_COLUMN:
      ...
      var columnIndexDragged = getColumnIndex(
        state.columns,
        action.idcolumnDragged
      );

      var cardIndex = getTaskIndex(
        state.columns,
        action.taskId,
        columnIndexDragged
      );
